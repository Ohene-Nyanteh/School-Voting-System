...
export default function GirlsCompoundPrefectPage() {
  ...
  const getCompoundPrefectPage = async () => {
    const records = await pb.collection("Prefects").getFullList({
      sort: "-created",
      filter: `type = "Compound Prefect" && Gender="Female"`,
    });
  ...
  return (
    <div className="w-full h-screen flex justify-center items-center">
      <div className="w-fit p-6 rounded shadow flex flex-col justify-center border">
        ...
        <span className="text-2xl text-center font-bold">Compound Prefect</span>
        ...
        disabled={voted ? false : true}
          ...
          onClick={() => navigate("/EntertainmentPrefectPage")}
        >
          Next
        </button>
      </div>
