...
export default function BoysSchoolPrefect() {
  ...
  const getSchoolPrefectData = async () => {
    const records = await pb.collection("Prefects").getFullList({
      sort: "-created",
      filter: `type = "School Prefect" && Gender="Male"`,
    });
  ...
  return (
    <div className="w-full h-screen flex justify-center items-center">
      <div className="w-fit p-6 rounded shadow flex flex-col justify-center border">
        ...
        <span className="text-2xl font-bold text-center">School Prefect</span>
        <span className="text-xl text-center ">Boys</span>
        <div className="p-2 flex flex-col gap-2 overflow-y-auto">
          {record && (
            <div className="flex flex-col gap-4">
              {record.map((r: any, key: number) => (
                <div key={key} className="flex bg justify-between items-center hover:bg-gray-200 hover:shadow-sm p-4 rounded px-3">
